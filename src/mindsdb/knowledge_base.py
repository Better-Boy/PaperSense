...
class KnowledgeBase:
    ...
    def insert(
        ...
        logger.info(
            ...
            successful_batches, total_batches, success_rate * 100
        )
    ...
    def get_stats(self, name: str) -> Dict[str, Any]:
        ...
        Returns:
            Dictionary containing statistics, with 'record_count' key
        """
        try:
            stats_query = f"SELECT COUNT(*) as record_count FROM {name};"
            result = self.conn.execute_query(stats_query)
            
            if result and len(result) > 0:
                return {"record_count": result[0].get("record_count", 0)}
        except Exception as e:
