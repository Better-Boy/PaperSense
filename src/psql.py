...
class PostgresHandler:
    ...
    def insert_article(self, article_data: Dict[str, Any]) -> None:
        ...
        insert_query = f"""
            INSERT INTO {table_name} (
                ...
                article_id, text, abstract, title
            ) VALUES (
                %(authors)s, %(categories)s, %(published_year)s, %(primary_category)s,
                %(article_id)s, %(text)s, %(abstract)s, %(title)s
            )
        """
        ...
        try:
            ...
            logger.debug(
                f"Successfully inserted article: {article_data.get('article_id')}"
            )
        except PostgresError as e:
            logger.error(
                f"Failed to insert article {article_data.get('article_id')}: {e}"
            )
    ...
    def get_paper_from_psql(self, arxiv_id) -> dict:
        logger.info(
            f"Querying {config.psql.database}.{config.psql.table_name} for {arxiv_id}"
        )
        select_query = f"SELECT * FROM {config.psql.table_name} where article_id = '{arxiv_id}';"
        res = self.execute_query(select_query, {}, True)
        if res:
            res = dict(res[0])
            del res["id"]
            return res
