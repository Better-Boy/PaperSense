...
class StressTest(MindsDBKnowledgeBaseTest):
    ...
    def start(
        ...
        console.print(
            Panel("[bold red]Starting Stress Tests[/bold red]", box=box.DOUBLE)
        )
        ...
        for size in self.stress_config.data_sizes:
            ...
            try:
                ...
                ingestion_result = self.ingest_data(
                    stress_data, batch_size=self.stress_config.batch_size
                )
                self.results["data_volume_stress"][size] = ingestion_result

                if (
                    ingestion_result["chunks_per_second"] < 10
                    ...
                    console.print(
                        f"[red]⚠️  Performance degradation detected at {size:,} records[/red]"
                    )
                    break
        ...
        for user_count in self.stress_config.concurrent_users:
            ...
            try:
                console.print(
                    f"[blue]Testing with {user_count} concurrent users[/blue]"
                )
                concurrent_exec_result = self.run_concurrent_queries(
                    queries, user_count
                )  # Use subset of queries
                self.results["concurrent_user_stress"][user_count] = (
                    concurrent_exec_result
                )

                # Check for failure conditions
                if (
                    concurrent_exec_result["avg_latency_ms"] > 5000
                    or concurrent_exec_result["successful_queries"]
                    < len(queries) * user_count * 0.95
                ):
                    self.results["failure_points"].append(
                        {
                    ...
                    console.print(
                        f"[red]⚠️  Performance issues detected with {user_count} users[/red]"
                    )
                    break
    ...
    def _write_stress_report(self, f):
        ...
        f.write(
            f"- **Max Concurrent Users**: {self.stress_config.max_concurrent_users}\n"
        )
        ...
        if failure_points:
            ...
            for failure in failure_points:
                f.write(
                    f"- **{failure['test_type'].replace('_', ' ').title()}**: Failed at {failure['failure_point']} - {failure['reason']}\n"
                )
        else:
            f.write(
                "- **System Stability**: All stress tests completed successfully within configured limits\n"
            )
        ...
        if "data_volume_stress" in self.results:
            ...
            f.write(
                "| Data Size | Ingestion Time (s) | Throughput (chunks/s) | Memory Usage (MB) | Status | Batch Size |\n"
            )
            f.write(
                "|-----------|--------------------|-----------------------|-------------------|--------|------------|\n"
            )
            ...
            for size, test_results in self.results["data_volume_stress"].items():
                ...
                memory_usage = test_results.get("system_metrics", {}).get(
                    "peak_memory_mb", 0
                )
                status = (
                    "✅ Pass" if test_results.get("total_errors", 0) == 0 else "❌ Fail"
                )

                f.write(
                    f"| {size:,} | {ingestion_time:.2f} | {throughput:.2f} | {memory_usage:.2f} | {status} | 50 |\n"
                )
            ...
            throughputs = [
                (
                    size,
                    self.results["data_volume_stress"][size].get(
                        "chunks_per_second", 0
                    ),
                )
            ...
            if len(throughputs) > 1:
                ...
                for size, throughput in throughputs[1:]:
                    if baseline_throughput > 0:
                        degradation = (
                            (baseline_throughput - throughput) / baseline_throughput
                        ) * 100
                        if degradation > 20:
                            f.write(
                                f"- **Performance Degradation**: {degradation:.1f}% decrease in throughput at {size:,} records\n"
                            )
                            break
        ...
        f.write("\n## Concurrent User Stress Test\n\n")
        f.write(
            "This test evaluates system performance under increasing concurrent load.\n\n"
        )

        if "concurrent_user_stress" in self.results:
            f.write("### Concurrent Load Results\n\n")
            f.write(
                "| Concurrent Users | Throughput (QPS) | Avg Latency (ms) | Success Rate (%) | Status |\n"
            )
            f.write(
                "|------------------|------------------|-------------------|------------------|--------|\n"
            )
            ...
            for users, test_results in self.results["concurrent_user_stress"].items():
                ...
                success_rate = (
                    test_results.get("successful_queries", 0)
                    / test_results.get("total_queries", 1)
                ) * 100
                status = (
                    "✅ Pass" if success_rate > 95 and avg_latency < 5000 else "❌ Fail"
                )

                f.write(
                    f"| {users} | {throughput:.2f} | {avg_latency:.2f} | {success_rate:.1f} | {status} |\n"
                )
            ...
            results_list = [
                (users, self.results["concurrent_user_stress"][users])
            ...
            if len(results_list) > 1:
                for i, (users, test_results) in enumerate(results_list[1:], 1):
                    ...
                    if prev_throughput > 0:
                        throughput_increase = (
                            (curr_throughput - prev_throughput) / prev_throughput
                        ) * 100
                        if throughput_increase < 10:  # Less than 10% improvement
                            f.write(
                                f"- **Saturation Point**: System reaches saturation around {users} concurrent users\n"
                            )
                            break
