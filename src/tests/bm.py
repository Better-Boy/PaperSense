...
class BenchmarkTest(MindsDBKnowledgeBaseTest):
    ...
    def start(
        ...
        console.print(
            Panel("[bold green]Starting Benchmark Tests[/bold green]", box=box.DOUBLE)
        )
        ...
        for batch_size in self.bench_config.batch_sizes:
            ...
            for iteration in range(self.bench_config.iterations):
                console.print(
                    f"[yellow]Iteration {iteration + 1}/{self.bench_config.iterations}[/yellow]"
                )
    ...
    def _write_benchmark_report(self, f):
        ...
        f.write(
            f"**Test Duration**: {self.results.get('test_start_time', '')} - {self.results.get('test_end_time', '')}\n\n"
        )
        ...
        f.write(
            f"- **System**: {psutil.cpu_count()} CPU cores, {psutil.virtual_memory().total / (1024**3):.1f} GB RAM\n"
        )
        f.write(
            f"- **Test Configuration**: {self.bench_config.iterations} iterations per test\n"
        )
        f.write(
            f"- **Batch Sizes Tested**: {', '.join(map(str, self.bench_config.batch_sizes))}\n"
        )
        ...
        if best_batch_size:
            f.write(
                f"- **Optimal Batch Size**: {best_batch_size} (achieving {best_throughput:.2f} chunks/second)\n"
            )
        ...
        if "query_tests" in self.results:
            ...
            f.write(
                f"- **Query Performance**: Average latency {avg_latency:.2f}ms, P95 latency {p95_latency:.2f}ms\n"
            )
        ...
        if "ingestion_tests" in self.results:
            ...
            f.write(
                "| Batch Size | Avg Throughput (chunks/s) | Avg Time per Chunk (ms) | Avg Time per 1000 Chunks (ms) | Memory Usage (MB) |\n"
            )
            f.write(
                "|------------|---------------------------|-------------------------|-------------------------------|-------------------|\n"
            )
            ...
            for batch_size, test_results in self.results["ingestion_tests"].items():
                ...
                f.write(
                    f"| {batch_size} | {throughput:.2f} | {time_per_chunk:.2f} | {time_per_1000:.2f} | {memory_usage:.2f} |\n"
                )
            ...
            for batch_size, test_results in self.results["ingestion_tests"].items():
                ...
                f.write(
                    f"- **Average Throughput**: {avg_metrics.get('chunks_per_second', 0):.2f} ± {avg_metrics.get('chunks_per_second_std', 0):.2f} chunks/second\n"
                )
                f.write(
                    f"- **Average Time per Chunk**: {avg_metrics.get('avg_time_per_chunk_ms', 0):.2f} ± {avg_metrics.get('avg_time_per_chunk_ms_std', 0):.2f} ms\n"
                )
                f.write(
                    f"- **Average Time per 1000 Chunks**: {avg_metrics.get('avg_time_per_1000_chunks_ms', 0):.2f} ± {avg_metrics.get('avg_time_per_1000_chunks_ms_std', 0):.2f} ms\n"
                )

                if iterations:
                    first_iteration = iterations[0]
                    f.write(
                        f"- **Total Files Processed**: {first_iteration.get('total_files', 0)}\n"
                    )
                    f.write(
                        f"- **Total Chunks**: {first_iteration.get('total_chunks', 0)}\n"
                    )
                    f.write(
                        f"- **Average Chunks per File**: {first_iteration.get('avg_chunks_per_file', 0):.2f}\n"
                    )
                    f.write(
                        f"- **Success Rate**: {(first_iteration.get('successful_chunks', 0) / first_iteration.get('total_chunks', 1)) * 100:.1f}%\n"
                    )
        ...
        if "query_tests" in self.results:
            ...
            f.write("|--------|------------|\n")
            f.write(
                f"| Average Latency | {latency_stats.get('avg_latency_ms', 0):.2f} |\n"
            )
            f.write(
                f"| Median Latency | {latency_stats.get('median_latency_ms', 0):.2f} |\n"
            )
            ...
            f.write(
                f"- **Successful Queries**: {query_results.get('successful_queries', 0)}\n"
            )
            f.write(f"- **Failed Queries**: {query_results.get('failed_queries', 0)}\n")
            f.write(
                f"- **Success Rate**: {(query_results.get('successful_queries', 0) / query_results.get('total_queries', 1)) * 100:.1f}%\n"
            )
            f.write(
                f"- **Queries per Second**: {query_results.get('queries_per_second', 0):.2f}\n\n"
            )
        ...
        if hasattr(self, "recommendations") and self.recommendations:
            ...
            f.write(
                "No specific recommendations generated. System performance appears optimal.\n\n"
            )
        ...
        if baseline_metrics:
            ...
            f.write(
                "To establish baseline metrics, save the current test results as baseline for future comparisons.\n\n"
            )
