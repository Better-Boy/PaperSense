...
class SearchFilters(BaseModel):
    ...
    year: Optional[int] = Field(
        None, ge=1991, le=2030, description="Publication year filter", example=2023
    )
...
class PaperResult(BaseModel):
    ...
    article_id: str = Field(..., description="ArXiv paper ID", example="2301.01234")
    title: str = Field(
        ..., description="Paper title", example="Attention Is All You Need"
    )
    authors: str = Field(
        ..., description="Paper authors", example="Ashish Vaswani, Noam Shazeer"
    )
    abstract: str = Field(
        ...,
    ...
    categories: str = Field(
        ..., description="ArXiv categories", example=["cs.CL", "cs.AI"]
    )
    published_year: str = Field(..., description="Publication year", example="2017")
    relevance: Optional[float] = Field(
        None, ge=0.0, le=1.0, description="Relevance score (0-1)", example=0.95
    )
...
class SearchResponse(BaseModel):
    ...
    results: List[PaperResult] = Field(
        default_factory=list, description="List of paper search results"
    )
    ...
    class Config:
        ...
        model_config = {
            "json_schema_extra": {
                ...
                "filters": {"category": "cs.ai", "year": 2017},
            }
        }
