...
def load_config_from_yaml(config_path: str) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.debug(
            f"Successfully loaded YAML configuration with {len(data) if data else 0} top-level keys"
        )
...
def _get_env_overrides() -> Dict[str, Any]:
    ...
    if mindsdb_port:
        try:
            ...
            logger.warning(
                f"Invalid MINDSDB_PORT value '{mindsdb_port}', must be an integer"
            )

    # OpenAI API Key override
    openai_key = os.getenv("OPENAI_API_KEY")
    if openai_key:
        overrides.setdefault("app", {})["openai_api_key"] = openai_key
        ...
        if env_value is not None:
            ...
            if config_key == "port":
                try:
                    ...
                    logger.warning(
                        f"Invalid POSTGRES_PORT value '{env_value}', must be an integer"
                    )
                    continue
            else:
    ...
    if postgres_overrides_found:
        logger.debug(
            f"Found PostgreSQL overrides: {', '.join(postgres_overrides_found)}"
        )
