...
from src.MindsDBMiddleware import manager, knowledge_base, ai_table
...
class WarmUp:
    ...
    def __init__(
        ...
        psql: psql.PostgresHandler,
        ai_table: ai_table.AITable
    ) -> None:
    ...
    def create_psql_table(self) -> None:
        ...
        try:
            logger.info(
                f"Creating PostgreSQL table '{config.psql.table_name}' if it doesn't exist"
            )
            ...
            logger.info(
                f"PostgreSQL table '{config.psql.table_name}' created/verified successfully"
            )

        except Exception as e:
            logger.error(
                f"Failed to create PostgreSQL table '{config.psql.table_name}': {e}"
            )
            raise
    ...
    def create_mindsdb_psql_db_connection(self, db_name: str, engine: str) -> None:
        ...
        try:
            ...
            if db_name not in existing_databases:
                logger.info(
                    f"Database '{db_name}' not found in MindsDB, creating connection"
                )
                ...
                logger.info(
                    f"MindsDB database connection '{db_name}' created successfully"
                )
            else:
            ...
            logger.error(
                f"Failed to create MindsDB database connection '{db_name}': {e}"
            )
            raise
    ...
    def _load_sample_data(self) -> List[Dict[str, Any]]:
        ...
        try:
            with open(data_file_path, "r", encoding="utf-16") as file:
                data = json.load(file)
    ...
    def _process_record(self, record: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if not text_content:
                logger.warning(
                    "Record contains no 'text' field, skipping text processing"
                )
                return
            ...
            logger.debug(
                f"Chunking text content (length: {len(text_content)} characters)"
            )
            chunks = utils.chunk_text(text_content, chunk_size=1500, chunk_overlap=300)
            enriched_chunks = [chunk | metadata for chunk in chunks]

            logger.info(
                f"Processing {len(enriched_chunks)} chunks for record {metadata['article_id']}"
            )
            ...
            logger.info(
                f"Successfully inserted {len(enriched_chunks)} chunks to knowledge base"
            )
    ...
    def check_embedding_compatability(self):
        if config.kb.embedding_model == "text-embedding-3-large":
            raise Exception(
                "Invalid embedding model for pgvector. Please use 'text-embedding-3-small' or 'text-embedding-ada-002'."
            )
    ...
    def start(self) -> None:
        ...
        try:
            logger.info(
                "Step 1: Creating MindsDB PSQL vector database connection if enabled"
            )
            if config.kb_storage.enable_pg_vector:
                self.check_embedding_compatability()
                self.create_mindsdb_psql_db_connection(
                    db_name=config.kb_storage.pg_vector_database, engine="pgvector"
                )
            ...
            self.create_mindsdb_psql_db_connection(
                db_name=config.psql.database, engine="postgres"
            )
            ...
            logger.info("Step 6: Create AI tables")
            self._ai_table.create_ai_tables()
